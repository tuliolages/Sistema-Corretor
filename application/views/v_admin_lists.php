...
<?php foreach ($lists_data as $list) {
	$id = $list['id_lista'];
...
<?php if ($list['data_finalizacao']) { ?>	Prazo: <strong><?=$this->datahandler->translate_date_format($list['data_finalizacao'])?></strong><br/><?php } ?>
<?php if ($list['data_lancamento']) { ?>	Inicio: <strong><?=$this->datahandler->translate_date_format($list['data_lancamento'])?></strong><br/><?php } ?>
<?php if ($list['data_fim_revisao']) { ?>	Revisão (prazo): <strong><?=$this->datahandler->translate_date_format($list['data_fim_revisao'])?></strong><br/><?php } ?>
<?php if ($list['data_inicio_revisao']) { ?>	Revisão (início): <strong><?=$this->datahandler->translate_date_format($list['data_inicio_revisao'])?></strong><br/><?php } ?>
